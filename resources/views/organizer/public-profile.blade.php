...
@section('content')
    ...
    @php
        $banner = $organizer->media->where('collection', 'banner')->last();

        $avatar = $organizer->media->where('collection', 'avatar')->last();
    @endphp

    <!-- 🔹 HERO / COVER -->
    <section class="relative h-72 bg-gradient-to-r from-purple-900 via-slate-900 to-cyan-900 overflow-hidden">

        {{-- Banner Image --}}
        @if ($banner)
            <img src="{{ $banner->url }}" class="absolute inset-0 w-full h-full object-cover opacity-40">
        @endif
        ...
        <div class="relative max-w-7xl mx-auto px-6 h-full flex items-end pb-8">
            ...
            <div class="flex items-center space-x-6">
                ...
                <!-- Avatar -->
                <img src="{{ $avatar?->url ??
                    'https://ui-avatars.com/api/?name=' . urlencode($organizer->name) . '&background=0F172A&color=22D3EE&size=128' }}"
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-4 mt-3">
                        <span class="text-yellow-300">
                            ⭐ {{ $organizer->rating ?? '4.9' }} / 5
                        </span>
                        <span class="text-gray-400">
                            • Member since {{ $organizer->created_at->format('Y') }}
                        </span>
                    </div>
                </div>
